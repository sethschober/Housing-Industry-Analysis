...
def elimination_by_code(series, code_to_keep):
    ...
    This is used when simplifying data imported from King Couunty dataset,
    which provides irrelevant data that needs to be eliminated. By setting
    invalid values to np.nan, the 'na' values can later be dropped from
    the DataFrame

    Parameters:
    -----------
    series : Pandas Series containing the data to be filtered
    code_to_keep : the only value in series that is to be retained.
        Others are replaced by np.nan
...
def identify_latest_sale(docdates, parcel_ids):
    ...
    The function returns a True/False series indicating whether the timestamp
    associated with each row is the most recent for each parcel.

    Parameters:
    -----------
    docdates : series containing the dates to compare, corresponding to the
        parcel_ids
    parcel_ids : series containing the parcel_id associated with each docdate

    Returns:
    --------
    latest_parcel_sale : True/False series showing whether the docdate is
        the most recent date for each parcel_id
    ...
    for i, parcel_id in enumerate(data.Parcel_ID):
        relevant_docdates = data.loc[
            data.Parcel_ID == parcel_id, "DocumentDate"
        ]
...
def avg_price_for_duped_parcels(data):
    ...
    Returns:
    --------
    data : input's SalePrice column is replaced with the average
        of SalePrice *for that Parcel_ID*
    ...
    for i, ind in enumerate(dupes.index):
        ...
        parcels_w_parcel_id = data.loc[
            data.Parcel_ID == parcel_id, "SalePrice"
        ]

        # Calculate average price for that parcel
        avg_price_for_id = parcels_w_parcel_id.values.mean()

        # Loop over all parcels with that Parcel_ID and assign the avg price
...
def get_lookups(LUType):
    ...
    Returns:
    --------
    result : dictionary with the King County lookup code as index and
        corresponding definition as value
    """

    # Import lookup database
    df_lookup = pd.read_csv(
        os.path.join("..", "..", "data", "raw", "EXTR_LookUp.csv"), dtype="str"
    )
...
def cut_extremes(series, n):
    ...
    return series.apply(
        lambda x: np.nan if x < min_ else np.nan if x > max_ else x
    )
...
def stepwise_selection(
    ...
    SOURCE: FUNCTION TAKEN DIRECTLY FROM FLATIRON COURSE MATERIAL
    (https://github.com/learn-co-curriculum/dsc-model-fit-linear-regression-lab)
    with minor edits
    ...
    while True:
        ...
        if worst_pval > threshold_out:
            ...
            worst_feature = pvalues.index[
                worst_feature
            ]  # SETH ADDED FROM ORIGINAL TO RESOLVE RUNTIME ERRORS
...
def produce_model(df, x, y, verbose=True):
    ...
    Parameters:
    -----------
    df : Pandas DataFrame used a source for model. *Must* include the columns
        provided as x and y
    ...
    df : DataFrame containing the columns provided in x and y, the data
...
def check_assumptions(model, df, y, verbose=False, feature_to_plot=False):
    ...
    Essential in determining the validity of a linear regression model by
    providing information about the four key assumptions in order to
    evaluate model performance: linearity, normality of residuals,
    homoscedacity, and independence. Optionally provide graphical
    representations annd/or printouts of key metrics, in addition to
    returning a DataFrame with those metrics

    Parameters:
    -----------
    model : statsmodels model object based on df with the output set as y
    df : DataFrame used to create the 'model' input
    y : the dependent variable of the model. Must be included in df and used
        to create model
    verbose : optionally print out metrics as calculations are made. These
        metrics are also returned
    feature_to_plot : select one column from df for which to plot the
        assumptions. Some graphs inherently show all features of the model,
        while others can only display one feature. In the latter case,
        this is the feature that is used. If a feature is not selected,
        no graphs are displayed.

    Returns:
    --------
    performance_metrics : DataFrame with one row containing: Model Output (y),
        Model Inputs (x), Linearity p-value, JB metric, JB p-value, Lagrange
        multiplier, Lagrange p-value, F-score, F-score p-value, Average VIF,
        Adjusted Rsquared
    """
    ...
    lm, lm_pvalue, fvalue, f_pvalue = homoscedacity(
        model, df, y, verbose, feature_to_plot
    )
...
def linearity(model, df, verbose, feature_to_plot):
    ...
    if feature_to_plot:
        ...
        for col in df_plotter.columns:
            ...
            # remove all features with only 2 unique values (binary /
            # one-hot enoded, for which graphical linearity is less relevant)
        ...
        plt.suptitle(
            "Investigating Linearity (Continuous Features Only)", y=1.05
        )
...
def normality(model, df, verbose, feature_to_plot):
    ...
    df : data used to create model
    verbose : optionally print metrics to console
    feature_to_plot : optionally display graph of residuals for the feature
    ...
    if verbose:
        print(
            "Normality of Residuals (where null hypothesis = normality):"
            + " JB stat={}, JB stat p-value={}".format(jb[0], jb[1])
        )
    ...
    if feature_to_plot:
        ...
        # QQ-plot
        sm.graphics.qqplot(
            df[feature_to_plot], line="45", fit=True, ax=axes[0]
        )
...
def homoscedacity(model, df, y, verbose, feature_to_plot):
    ...
    df : data used to create model
    verbose : optionally print metrics to console
    feature_to_plot : optionally display graph of residuals for feature
    ...
    if verbose:
        print(
            "Homoscedacity (where null hypothesis = homoscedastic): "
            + "lagrange p-value={} and f-value's p-value={}".format(
                lm_pvalue, f_pvalue
            )
        )
    ...
    if feature_to_plot:
        ...
        plt.plot(
            [df[feature_to_plot].min(), df[feature_to_plot].max()],
            [0, 0],
            color="black",
        )
...
def independence(model, df, y, verbose, feature_to_plot):
    ...
    df : data used to create model
    verbose : optionally print metrics to console
    feature_to_plot : optional display graph of residuals for select feature
    ...
    if len(features) == 1:
        ...
        df_vif["VIF"] = [
            variance_inflation_factor(df.drop(y, axis=1).values, i)
...
def identify_continuous_features(df):
    """
    Isolate columns with continuous features, excluding columns
    ...
    Returns:
    --------
    df_continous_features : DataFrame with columns eliminated in
        cases of few distinct values
    ...
    for col in df.columns:
        ...
        # Subjective threshold to eliminate discrete features
        # w/few unique values
        if ct > 20:

            # Add to list of continuous features if relevant
            if (isinstance(df[col][0], np.int64)) or (
                isinstance(df[col][0], np.float64)
            ):
